...
func calculateMinimumHP(dungeon [][]int) int {
	...
	for i := 0; i < len(dungeon); i++ {
		dArr = append(dArr, [][]int{})
		dArrSum = append(dArrSum, [][]int{})
		for j := 0; j < len(dungeon[i]); j++ {
			dArr[i] = append(dArr[i], []int{})
			dArrSum[i] = append(dArrSum[i], []int{})
		}
	}
	dArr[0][0] = []int{dungeon[0][0]}
	dArrSum[0][0] = []int{dungeon[0][0]}

	for i := 0; i < len(dungeon); i++ {
		for j := 0; j < len(dungeon[i]); j++ {
			bigEl := 0
			bigElSum := 0
			bestEls := make(map[int]int)
			for l, e := range dArr[i][j] {
				if l == 0 || e > bigEl {
					bigEl = e
					bigElSum = dArrSum[i][j][l]
				}
				if _, ok := bestEls[e]; ok {
					if bestEls[e] < dArrSum[i][j][l] {
						bestEls[e] = dArrSum[i][j][l]
					}
				} else {
					bestEls[e] = dArrSum[i][j][l]
				}
			...
			dArrSum[i][j] = []int{}
			// fmt.Println(bestEls)
			for k, v := range bestEls {
				// fmt.Println(v, bigElSum)
				if v >= bigElSum {
					dArr[i][j] = append(dArr[i][j], k)
					dArrSum[i][j] = append(dArrSum[i][j], v)
				}
			}
			for k := 0; k < len(a); k++ {
				if i+a[k][0] < len(dungeon) && j+a[k][1] < len(dungeon[i]) {
					for l := 0; l < len(dArr[i][j]); l++ {
						var val int
						if dArrSum[i][j][l]+dungeon[i+a[k][0]][j+a[k][1]] < dArr[i][j][l] {
							val = dArrSum[i][j][l] + dungeon[i+a[k][0]][j+a[k][1]]
						} else {
							val = dArr[i][j][l]
						}
						dArr[i+a[k][0]][j+a[k][1]] = append(dArr[i+a[k][0]][j+a[k][1]], val)
						dArrSum[i+a[k][0]][j+a[k][1]] = append(dArrSum[i+a[k][0]][j+a[k][1]], dArrSum[i][j][l]+dungeon[i+a[k][0]][j+a[k][1]])
					}
				}
			}
		}
	}
	for i, e := range dArr[len(dArr)-1][len(dArr[0])-1] {
		if i == 0 || e > bestPath {
			bestPath = e
		}
	}
